...
! !USES:
   ...
   use mem_BenBac, ONLY: p_qnHc=>p_qnc,p_qpHc=>p_qpc,p_suR6
   use mem_BenOrganism, ONLY: p_qnYc=>p_qn,p_qpYc=>p_qp
   ...
   use mem_PelBac, ONLY: CalcNitrifiers
   ...
   use mem_Param, ONLY: CalcPelagicFlag,CalcBenthicFlag,p_small, &
                        ...
                        CalcBenPhyto, CalcSuspensionFeeders, &
                        p_small,p_poro,p_d_tot,p_d_tot_2,p_1d_poro
   use string_functions, ONLY: getseq_number,empty,set_nco_style
   use mem_globalfun,   ONLY: IntegralExpDist_vector,IntegralExp
   use global_interface,only:CouplingBioSilt
   IMPLICIT NONE
!
! !INPUT PARAMETERS:
...
! !LOCAL VARIABLES:
   ...
   namelist /bfm_ben_init_nml/calc_init_bennut_states,p_poro0, &
                           ...
                           Q1c0, Q11c0, Q21c0, Q2c0, Q12c0,Q22c0, Qt6c0,     &
                           Q6s0,Q16s0,K3n0, G4n0, H1c0,          &
                           H2c0, H3c0, HNc0,Hac0, K1p0, K11p0, K21p0,   &
                           K4n0, k14n0, K24n0, K6r0,K16r0,K26r0,K5s0,    &
                           K15s0,&
...
!   if (setup >=2) then
      ...
      Dfm=0.1e-6
      Dcm=0.1e-6
      write(LOGUNIT,*) "init_var_bfm Mf5s,Mf15s,Mf15s =",Mf5s,Mf15s,Mf15s 
!     irrenh_l=DONE
      write(LOGUNIT,*)'initial value DfmDcm=',Dfm,Dcm
      x_scalar= CalculateBenPhyto(iiC,INTEGRAL,1,ZERO,Dfm(1),inverse=0.5D+00)
      Dlm=x_scalar
      Kfp4n=K4n*min(1.0,Dlm/D1m0)
      Kfp3n=K3n
      ...
      G2o  = G2o0
      beta=1.0/D6m0
      x_scalar=IntegralExp(beta,p_d_tot)
      Q6c=Qt6c0/x_scalar*Integralexp(beta,D1m0)
      Q16c  = Qt6c0-Q6c
      Qh26c=Qt6c0/x_scalar*exp(-beta*(p_d_tot+D2m0)*0.5)  * &
                                 integralexp(beta,0.5*p_d_tot-0.5*D2m0)
      sdQ6c=-p_suR6
      sdQ6n=-p_suR6
      sdQ6p=-p_suR6
      sdQ6s=-p_suR6
      ...
      Mf5s=K5s/D1m/p_poro(1)/5.0
      Mf15s=Mf5s;Mf25s=Mf5s
      DTm=2.0*D1m
      DSm=-_ONE_

      if ( sw_BenSilt==1 .or.sw_BenSilt==2) then
        DSM=p_d_tot
        QSx=CouplingBioSilt(-1)
        alpha=_ONE_/DSM
        r_xy1=p_SampleDepth
        r_xy2=p_d_tot
...
!       write(LOGUNIT,*) 'init qsc=',BP1s(1)/Bp1c(1)
      ...
      call init_cnps(c=Y1c,n=Y1n,p=Y1p,nc=p_qnYc(1),pc=p_qpYc(1))
      call init_cnps(c=Y2c,n=Y2n,p=Y2p,nc=p_qnYc(2),pc=p_qpYc(2))
      call init_cnps(c=Y4c,n=Y4n,p=Y4p,nc=p_qnYc(3),pc=p_qpYc(3))
      call init_cnps(c=Y5c,n=Y5n,p=Y5p,nc=p_qnYc(4),pc=p_qpYc(4))
      call init_cnps(c=Yy3c,n=Yy3n,p=Yy3p,nc=p_qnY3c,pc=p_qpY3c)
      call init_cnps(c=Y3c,n=Y3n,p=Y3p,nc=p_qnY3c,pc=p_qpY3c)
...
!    end if
      ...
      call init_cnps(c=Q16c,n=Q16n,p=Q16p,s=Q16s, &
                                   nc=p_qnQIc,pc=p_qpQIc,sc=p_qsQIc)
      call init_cnps(c=Qh26c,n=Qh26n,p=Qh26p,s=Qh26s, &
                                   nc=p_qnQIc,pc=p_qpQIc,sc=p_qsQIc)
      if ( Q6s0.gt.ZERO) Q6s=Q6s0
      if ( Q6s0.gt.ZERO) Q6s=Q6s0
      if ( Q16s0.gt.ZERO) Q16s=Q16s0
      Qun=Q1n*0.0005;Q1un=Q11n*0.0005;Q2un=Q21n*0.0005;Qfpun=Qun
...
!          call InitBenthicNutrientDynamics(calc_init_bennut_states)
   ...
   if ( CalcNitrifiers) D3STATE(ppBac,:)=0.05*D3STATE(ppB1c,:)
   ...
   do j = 1,iiSuspensionFeeders
      iiLastElement = iiP
      if (.NOT.CalcSuspensionFeeders(j)) then
         do i = iiC,iiLastElement
            D2STATE(ppSuspensionFeeders(j,i),:) = p_small
         end do
      end if
   end do
