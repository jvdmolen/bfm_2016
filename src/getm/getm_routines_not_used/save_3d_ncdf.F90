...
!
! !BFM
!   --code added to save BFM- variables in output file (=result file):
!     1. state variables
!     2. diagnostic variables
!    Only BFM-variables defined in the list in bio_bfm.nml will be saved.
!
! !USES:
   use netcdf
   use exceptions
...
#else
  use bio_var, only: c1dimz,bio_setup
  use variables_bio_3d, only: cc3d,ccb3d,cc3d_out,ccb3d_out,ffb,ffp
  use bfm_output, only: &
   ...
   var_ave,var_ids,var_names 
  use coupling_getm_bfm,only :fill_diagn_bfm_vars ,DIAG_AVERAGE,DIAG_RESET
#ifdef INCLUDE_DIAGNOS_PRF
   use bio_var, only: numbc_prf
   use bfm_output, only: stPRFDiagS,stPRFDiagE
   use variables_bio_3d, only: ccb3d_prf
#endif
#endif
...
#endif
#ifdef INCLUDE_DIAGNOS_PRF
   REALTYPE, allocatable,target    :: ws_prf(:,:,:)
#endif
...
! rotated zonal and meridional velocities
...
#ifdef BFM_GOTM
   if ( bio_calc) then
      call fill_diagn_bfm_vars( DIAG_AVERAGE,.true., 0,0,c1dimz ,kmax,_ZERO_ )
#endif 
      ...
      do n=stPelStateS, stPelFluxE
        if ((var_ids(n) /= 0)  .and. var_ave(n) ) then   !BFM 
          ! Only values to be stored are copied in the diagnostic 4d-array
          k=k+1
          ffp=cc3d_out(:,:,:,k)
          call cnv_3d(imin,jmin,imax,jmax,kmin,kmax,az,ffp,bio_missing, &
                 imin,imax,jmin,jmax,0,kmax,ws)
          err = nf90_put_var(ncid, var_ids(n), ws(_3D_W_), start=start, count=edges)   !BFM
          if (err .NE.  NF90_NOERR) then
            string='when saving averaged diagnostic vars:'//trim(var_names(n))
            call netcdf_error(err, 'save_3d_ncdf',trim(string))
          endif
        end if                                                             !BFM
      end do
      do n=stPelStateS, stPelFluxE
        if ((var_ids(n) /= 0)  .and. (.not.var_ave(n))) then   !BFM 
          ...
          call cnv_3d(imin,jmin,imax,jmax,kmin,kmax,az,ffp,bio_missing, &
                  imin,imax,jmin,jmax,0,kmax,ws)
          err =nf90_put_var(ncid, var_ids(n),ws(_3D_W_),start=start,count=edges)
          if (err .NE.  NF90_NOERR) then
             string='when saving averaged diagnostic vars:'//trim(var_names(n))
             call netcdf_error(err, 'save_3d_ncdf',trim(string))
          endif
        ...
        do n=stBenStateS, stBenFluxE
          if ((var_ids(n) /= 0)  .and. var_ave(n)) then   !BFM 
            ...
            call cnv_2d(imin,jmin,imax,jmax,az,ffb,bio_missing, &
                    imin,jmin,imax,jmax,ws2d)
            err=nf90_put_var(ncid,var_ids(n),ws2d(_2D_W_), &
                                         start=start, count=edges)    !BFM
            if (err .NE.  NF90_NOERR) then
              string='when saving averaged diagnostic vars:' &
      ...
      call fill_diagn_bfm_vars( DIAG_RESET,.true., 0,0,c1dimz ,kmax,_ZERO_ )
   endif
#ifdef INCLUDE_DIAGNOS_PRF
   allocate(ws_prf(I2DFIELD,0:numbc_prf),stat=k)
   start(1) = 1
   start(2) = 1
   start(3) = 1
   start(4) = n3d
   ...
   do n=stPRFDiagS, stPRFDiagE
     if ((var_ids(n) /= 0)  .and. var_ave(n) ) then   !BFM 
       ! Only values to be stored are copied in the diagnostic 4d-array
       k=k+1
       call cnv_3d(imin,jmin,imax,jmax,kmin,numbc_prf,az,ccb3d_prf(:,:,:,k), &
              bio_missing, imin,imax,jmin,jmax,0,numbc_prf,ws_prf)
       err = nf90_put_var(ncid, var_ids(n), ws_prf(_2D_W_,0:numbc_prf), &
                                            start=start, count=edges)   !BFM
       if (err .NE.  NF90_NOERR) then
         string='when saving averaged diagnostic vars:'//trim(var_names(n))
         call netcdf_error(err, 'save_3d_ncdf',trim(string))
   ...
   do n=stPRFDiagS, stPRFDiagE
     if ((var_ids(n) /= 0)  .and. (.not.var_ave(n))) then   !BFM 
       ...
       err = nf90_put_var(ncid, var_ids(n), ws_prf(_2D_W_,0:numbc_prf), &
                                           start=start, count=edges)   !BFM
       if (err .NE.  NF90_NOERR) then
         STDERR 'size ws_prf=',size(ws_prf),xlen*ylen*(numbc_prf+1),xlen,ylen
         string='when saving diagnostic vars:'//trim(var_names(n))
         call netcdf_error(err, 'save_3d_ncdf',trim(string))
       endif
     end if                                                             !BFM
   end do
