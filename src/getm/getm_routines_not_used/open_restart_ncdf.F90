...
!
! !INTERFACE:
   subroutine open_restart_ncdf(fname,runtype,hotstart_bio)
!
! !DESCRIPTION:
...
!
! !BFM
!   Code to read all netcdf-BFM variable id's
!
! !USES:
   use netcdf
   use ncdf_restart
...
#ifdef BFM_GOTM
        status = nf90_get_att(ncid,NF90_GLOBAL,'FirstPelVar',varnam)
        if (status .eq. NF90_NOERR) &  
           status = nf90_inq_varid(ncid,varnam , start_id_3d)
        status = nf90_get_att(ncid,NF90_GLOBAL,'FirstBenVar',varnam)
        if (status .eq. NF90_NOERR) &  
           status = nf90_inq_varid(ncid,varnam , start_id_2d)
        !get id of numc: number of pelagic state variables in netcdf file
        if ( start_id_2d==0 .and.start_id_3d==0 ) then
          varnam="numc"   !JM added
          status = nf90_inq_varid(ncid, "numc", numc_id)
          if (status .NE. NF90_NOERR) go to 10
          !get id of numbc: number of benthic state variables in netcdf file
          varnam="numbc"   !JM added
          status = nf90_inq_varid(ncid, "numbc", numbc_id)
          if (status .NE. NF90_NOERR) go to 10
          ! get id of first pelagic state variable
          varnam="start_3d"   !JM added
          status = nf90_inq_varid(ncid, "start_3d", id_var_start_3d)
          if (status .NE. NF90_NOERR) go to 10
          ! get id of first bentic state variable
          varnam="start_2d"   !JM added
          status = nf90_inq_varid(ncid, "start_2d", id_var_start_2d)
          if (status .NE. NF90_NOERR) go to 10
        endif
#else
         varnam="bio"
