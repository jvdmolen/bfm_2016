...
!
! !INTERFACE:
   subroutine        short_wave_radiation(yday,hour,lat,lon,tcc, &
                     swr,dl)
   IMPLICIT NONE
!
! !DESCRIPTION:
...
!
!  !BFM
!   -routine changed in subroutine
!   - line added to calculate daylength
!   - 2 output variables (instead of 1): short wave radiation and daylength
!
! !SEE ALSO:
!  meteo.F90
!
...
! !OUTPUT PARAMETERS:
   ...
   REALTYPE, intent(OUT),optional       :: dl
!
! !REVISION HISTORY:
...
#if 1
   swr  = qtot*(1-0.62*tcc + .0019*sunbet)*(1.-albedo)
   if(swr .gt. qtot ) then
      swr  = qtot
   end if
#else
...
#ifdef BFM_GOTM
   if (present(dl)) then
      stdwin = 180./pi*acos( - tan(alat) * tan(sundec))
      dl = 2.0* stdwin /15.0
   endif
#endif
