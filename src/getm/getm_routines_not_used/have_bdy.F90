...
!
! ! BFM
!     code added to support easer tracking
!     bdy_to ddy_seq are filled with values
!     With these arrays in a easy way boundary conitions for track can be set. 
!
! !USES:
   use domain
   IMPLICIT NONE
...
!BOC
      ...
      do n = 1,NWB
         ...
         if (wi(n) .ge. imin+ioff .and. wi(n) .le. imax+ioff) then
            ...
            if(f .le. l) then
               ...
               do k=1,nsbv
                  if (bdy_map(k,1) .eq. wi(i)+ioff ) then
                      if (bdy_map(k,2) .eq. f+joff) then
                           bdy_index(nbdy) = k
#ifndef BFM_GOTM
                      endif
#else
                      ...
                      if (bdy_map(k,2) .eq. l+joff) bdy_to(nbdy) = k
#endif
                  end if
               end do
            end if
      ...
      do n = 1,NNB
         ...
         if (nj(n) .ge. jmin+joff .and. nj(n) .le. jmax+joff) then
            ...
            if(f .le. l) then
               ...
               bdy_2d_type(nbdy) = bdy_2d_type(m)
               do k=1,nsbv
                  if ( bdy_map(k,2) .eq. nj(i)+joff) then
                     if (bdy_map(k,1) .eq. f+ioff ) then
                           bdy_index(nbdy) = k
#ifndef BFM_GOTM
                      endif
#else
                     ...
                     if (bdy_map(k,1) .eq. l+ioff ) bdy_to(nbdy) = k
#endif
                  end if
               end do
            end if
      ...
      do n = 1,NEB
         ...
         if (ei(n) .ge. imin+ioff .and. ei(n) .le. imax+ioff) then
            ...
            if(f .le. l) then
               ...
               bdy_2d_type(nbdy) = bdy_2d_type(m)
               do k=1,nsbv
                  if (bdy_map(k,1) .eq. ei(i)+ioff ) then 
                      if (bdy_map(k,2) .eq. f+joff) then
                         bdy_index(nbdy) = k
#ifndef BFM_GOTM
                      endif
      ...
      do n = 1,NSB
         ...
         if (sj(n) .ge. jmin+joff .and. sj(n) .le. jmax+joff) then
            ...
            if(f .le. l) then
               ...
               bdy_2d_type(nbdy) = bdy_2d_type(m)
               do k=1,nsbv
                  if ( bdy_map(k,2) .eq. sj(i)+joff) then
                     if (bdy_map(k,1) .eq. f+ioff ) then
                           bdy_index(nbdy) = k
#ifndef BFM_GOTM
                      endif
   ...
   if (nbdy .gt. 0) then
    ...
    LEVEL2 'subdomain have_boundaries=',have_boundaries
#ifdef DEBUG
