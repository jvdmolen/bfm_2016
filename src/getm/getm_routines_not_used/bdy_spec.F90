...
!     bdy_to ddy_seq are re allocated
!     With these arrays in a easy way boundary conditions for track can be set. 
!
! !USES:
...
#IFDEF BFM_GOTM
      allocate(bdy_to(NOB),stat=rc)
      if (rc /= 0) stop 'bdy_spec: Error allocating memory (bdy_to)'
      allocate(bdy_seq(NOB),stat=rc)
      if (rc /= 0) stop 'bdy_spec: Error allocating memory (bdy_seq)'
#ENDIF
