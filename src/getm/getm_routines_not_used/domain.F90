...
! 
! !BFM 
!   extras array defind to support in a more easy way tracking
!
! !USES:
   use exceptions
...
#ifdef BFM_GOTM
   integer, allocatable                :: bdy_to(:),bdy_seq(:)
#endif
...
!  Do we want to do adjust the bathymetry
   ...
   STDERR "domain before update_2d_halo"
   call update_2d_halo(H,H,az,imin,jmin,imax,jmax,H_TAG,mirror=.false.)
   call wait_halo(H_TAG)
   STDERR "domain after update_2d_halo"
   ...
   STDERR "Domain before call_uv_depths"
   call uv_depths(vel_depth_method)
   STDERR "Domain after call_uv_depths"
...
!  western boundary - at present elev only
   do n=1,NWB
      ...
      if (wi(n) .gt. imin) az(imin:wi(n)-1,wfj(n):wlj(n))=0
...
!  northern boundary - at present elev only
   do n=1,NNB
      ...
      if( nj(n) .lt. jmax) az(nfi(n):nli(n),nj(n)+1:jmax) = 0
...
!  easter boundary - at present elev only
   do n=1,NEB
      ...
      if (ei(n) .lt. imax) az(ei(n)+1:imax,efj(n):elj(n))=0
...
!  southern boundary - at present elev only
   do n=1,NSB
      ...
      if (sj(n) .gt. jmin) az(sfi(n):sli(n),jmin:sj(n)-1) =0 
...
91 LEVEL2 'done set minimum depth'
   return
...
91 LEVEL2 'done adjust_bathymetry'
   return
