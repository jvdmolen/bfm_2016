...
!
! !OUTPUT PARAMETERS:
   character(len=19),intent(out),optional         :: str
!
! !REVISION HISTORY:
!  See module for log.
!
...
!
! !INTERFACE:
   subroutine  julian_minus_one_year(in,out,loop)
!
! !DESCRIPTION:
!
! !USES:
...
! !LOCAL VARIABLES:
!
  integer        ::yyyy,mm,dd,nsecs,jul1,secs1
!EOP
!-----------------------------------------------------------------------
!BOC
  
   call CalDat(in,yyyy,mm,dd)
   call JulDay(yyyy-1,mm,dd,out)
   call String2JulSecs(start,jul1,secs1)
   nsecs = time_diff(out,secs1,jul1,secs1)
   loop  = nint(nsecs/timestep)
   return
   end subroutine julian_minus_one_year
!EOC
!-----------------------------------------------------------------------
