...
! 
! !BFM
!   routine specialized to initalize a combination GETM/BFM  (=GOTM-ERSEM) model
!
! !USES:
   IMPLICIT NONE
!
...
#ifdef BFM_GOTM
   use output,only:test_if3doutput
   use getm_bio,only: init_calc_getm_bio
   use bdy_3d,only:postinit_bdy_3d
...
!
! !INPUT PARAMETERS:
   character(len=*),intent(IN)       :: dstr,tstr
!
! !DESCRIPTION:
...
#if (defined GETM_PARALLEL && defined INPUT_DIR)
   STDERR 'Initialise input_dir:',trim(input_dir)
#endif
!
...
!
   ...
   if(runtype .ge. 3) then
      FATAL 'initialize:getm not compiled for baroclinic runs'
      stop 'init_model()'
   end if
#endif
   ...
   if(runtype .ge. 2) then
      FATAL 'initialize:getm not compiled for 3D runs'
      stop 'init_model()'
   end if
#endif
...
! call all modules init_ ... routines
   ...
   if (runtype .gt. 1) then
      call init_3d(runtype,timestep,.FALSE.)
...
#ifdef GETM_BIO
      k=step_3d/M
      call init_getm_bio(NAMLST2,trim(input_dir) // 'getm_bio.inp',k)
      if (bio_calc) call init_rivers_bio
#endif
   end if
#endif
   ...
   if (hotstart) then
      ...
      STDERR "initialise after restart_bio", &
              size(ccb,dim=1),size(ccb,dim=2),ppDzm,ccb(1,ppDzm)
...
#ifdef BFM_GOTM
      k=step_3d/M
      call init_calc_getm_bio(k)
#endif
    endif

   if (save_initial .and. .not. dryrun) then
      call do_output(runtype,MinN-1,timestep,.true.,.true.)
   endif
#endif
