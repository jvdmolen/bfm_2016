...
!
! !INTERFACE:
   subroutine read_par_setup(fn,nprocs,myid_0,imax,jmax,iextr,jextr,  &
...
! !INPUT PARAMETERS:
   ...
   integer, intent(in)                 :: nprocs,myid_0  ! Number of jobs actually started.
...
!
! !LOCAL VARIABLES:
   integer                   :: myid_read,myid,myid_opti,i,j
   ...
   character(len=*),parameter:: optimal_seq="optimal_seq.dat"         
   integer, parameter        :: junit=88 ! check 87 KBK
   integer                   :: iostat,iostat2
   integer,dimension(0:nprocs-1) :: seqmach,seqpro
   logical                   :: exist
!
!EOP
!-------------------------------------------------------------------------
...
!        skip comments and empty lines
   ...
   inquire(file=optimal_seq,exist=exist)

   myid=myid_0
   if ( exist ) then
     open(unit=junit,file=optimal_seq,iostat=iostat)
     iline   = 0 ! Index for line number in file
     thislineok = 0 ! Flag for this line read OK
     do while (thislineok .eq. 0 .and. iostat == 0)
       read(junit,'(A)',iostat=iostat,end=1021,err=1011) line
!        skip comments and empty lines
       if (line(1:1) == '#' .or. line(1:1) == '!' .or. &
            len(trim(line)) == 0 ) then
       else
         read(line,*,err=1010,end=1020) seqmach(iline),seqpro(iline)
         iline=iline+1
         thislineok=0
         if (iline==nprocs) thislineok=1
         STDERR iline,trim(line)
       end if
     end do
     if (iline /= nprocs) then
       FATAL 'optimal_seq: Number of jobs do not match'
       FATAL '  Expected value ',nprocs
       FATAL '  Read value     ',iline
       stop
     end if
   endif
   close(junit) 
...
!   Try alternative old format
      ...
      if(myid_read .eq. myid) then
         ...
         if (exist) then
           do i=1,8
             j=neighbours(i)
             if (j.ge.0) neighbours(i)=seqpro(j)
           enddo
         endif
      end if
!
...
! Capture errors:
 ...
 1011 continue
   FATAL 'optimal_seq: Unexpected format at line ',iline
   stop
 ...
 1020 continue
   ...
   stop
 1021 continue
   FATAL 'optimal_seq: Premature EOF at line',iline
   stop
