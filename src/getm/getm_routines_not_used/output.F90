...
! !LOCAL VARIABLES:
   namelist /io_spec/ &
             ...
             save_meteo, save_numerical_analyses,restart_force
...
!BOC
   ...
   if(save_3d) then
      ...
      LEVEL2 'First=',first_3d,' step=',step_3d,' plus_3d=',plus_3d
   end if
#endif

   STDERR 'hotout I:',hotout
...
!  end if
   ...
   if(.not. dryrun) then
      ...
      select case (out_fmt)
         ...
         case (NETCDF)
            if (save_2d) then
                STDERR "save 2d: start"
                call init_2d_ncdf(out_f_2d,title,starttime)
            endif
#ifndef NO_3D
            if (save_3d) then
               STDERR "save 3d: start"
               call init_3d_ncdf(out_f_3d,title,starttime)
            endif
!           if (save_mean)  &
...
!BOP
!
! !IROUTINE: test_if3doutput - test if output is written at this loop
!
! !INTERFACE:
   subroutine test_if3doutput(n,MinN,MaxN,write_3d)
!
! !DESCRIPTION:
...
!BOC
   ...
   integer, save :: Ncall = 0
   Ncall = Ncall+1
   write(debug,*) 'test_if3doutput() # ',Ncall
#endif

   write_3d = save_3d .and. n .ge. first_3d  
   write_3d=write_3d .and. & 
                       mod(n-MinN+1-plus_3d,step_3d).eq.0 .and. n.le.MaxN
!  STDERR 'write_3d',write_3d,mod(n-MinN+1-plus_3d,step_3d)
   return
...
!BOP
!
! !IROUTINE: test_if2doutput - test if output is written at this loop
!
! !INTERFACE:
   subroutine test_if2doutput(n,MinN,MaxN,write_2d)
!
! !DESCRIPTION:
!  test if output is written at this timestep
!
...
!BOC
   ...
   integer, save :: Ncall = 0
   Ncall = Ncall+1
   write(debug,*) 'test_if2doutput # ',Ncall
#endif

   write_2d = save_2d .and. n .ge. first_2d  
   write_2d=write_2d .and. & 
                       mod(n-MinN+1,step_2d).eq.0 .and. n.le.MaxN

   return
   end subroutine test_if2doutput
...
!
! !INTERFACE:
   subroutine do_output(runtype,n,timestep,write_2d,write_3d)
!
! !DESCRIPTION:
...
! use coupling_getm_bfm,ONLY:restart_file_bio
  use output_restart_bio,ONLY:restart_file_bio
  use bio_var, only: bio_setup
  use getm_bio, only: hotstart_bio
  use time, only:julian_minus_one_year
...
!BOC
...
#ifdef GETM_BIO
#ifdef BFM_GOTM
               call restart_file_bio(WRITING,BINARY,RESTART,julianday,.false.)
#else
               if(bio_calc) then
   ...
   if (mode .eq. READING) then
      ...
      LEVEL3 'bio_calc=',bio_calc,' hostart_bio=',hotstart_bio
      LEVEL3 'hotin_bfm=',hotin_fmt
      LEVEL3 trim(fname)
            hot_bio=bio_calc
#ifdef BFM_GOTM
            hot_bio=hot_bio.and.hotstart_bio
            STDERR 'hot_bio=',hot_bio
            STDERR 'hotstart_bio=',hotstart_bio
#endif
      select case (hotin_fmt)
         case(NETCDF)
            call open_restart_ncdf(fname,runtype,hot_bio)
            call read_restart_ncdf(runtype,j,jd,secs,dt,hot_bio,llfull)
#ifdef BFM_GOTM
           if (hot_bio) then
              call restart_file_bio(READING,NETCDF,RESTART,jd,llfull)
           else
            STDERR "Initial fields from 1D-model are used!"
           endif
#endif
...
#ifdef BFM_GOTM
             if (hot_bio)  &
                 call restart_file_bio(READING,BINARY,RESTART,jd,.false.)
#endif
             if(bio_calc )  then
...
#if BFM_GOTM
      if ( restart_force) then
          call julian_minus_one_year(jd,julianday,loop)
          secondsofday=secs; timestep=dt;
      elseif (use_epoch) then
#else
      if (use_epoch) then
#ENDIF
