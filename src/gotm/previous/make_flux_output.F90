...
! !USES:
      ...
      use PelBenInteraction,ONLY:D2toD1,D3toD1
      use bio_bfm, only:CalcVertFluxAtLev
...
!BOC
      ...
      llcalc=.false.;nr=nr0;
      if ( mode == 2 ) nr=nr0+flx_cal_ben_start
      if (flx_option(nr) >=30) return ; ! options meant for calculation of 3dcalculations'   .
      klev=NO_BOXES ; if ( flx_CalcIn(nr) == iiBen)  klev=NO_BOXES_XY 
      hulp(:)=0.0
      if ( flx_CalcIn(nr) == iiBen) then
        if ( flx_option(nr)== 20 ) then
         !do i=flx_calc_nr(nr-1)+1,flx_calc_nr(nr)
         !  call CalcVertFluxAtLev(flx_states(i),flx_ostates(i), &
         !                                          nlev,Depth,dt,r)
         !  hulp(klev:klev)=hulp(klev:klev)+r* flx_SS(i)
         !enddo
          ...
          do i=flx_calc_nr(nr-1)+1,flx_calc_nr(nr)
            if (flx_SS(i) ==1 ) then
               hulp(1:klev)= hulp(1:klev) &
                    + flx_t(i) * D2SINK(1:klev,flx_states(i),flx_ostates(i))
            else
               hulp(1:klev)= hulp(1:klev) &
                   + flx_t(i) * D2SOURCE(1:klev,flx_states(i),flx_ostates(i))
            endif
          enddo
        endif
      elseif (flx_option(nr)== 10 ) then
        do i=flx_calc_nr(nr-1)+1,flx_calc_nr(nr)
            hulp(1:klev)= hulp(1:klev) + D3STATE(1:klev,flx_states(i))
        enddo
      elseif ( flx_option(nr)< 10 ) then
        do i=flx_calc_nr(nr-1)+1,flx_calc_nr(nr)
          if (flx_SS(i) ==1 ) then
             hulp(1:klev)= hulp(1:klev) &
                    + flx_t(i) * D3SINK(1:klev,flx_states(i),flx_ostates(i))
             ...
             hulp(1:klev)= hulp(1:klev) &
                  + flx_t(i) * D3SOURCE(1:klev,flx_states(i),flx_ostates(i))
             ! correcting for fluxes  to other subsystems
             if ( flx_states(i) ==flx_ostates(i)) then
                hulp(1)=hulp(1)-flx_t(i) *max(0.0D+00,&
                       PELBOTTOM(flx_states(i),1))/Depth(1)/SEC_PER_DAY
                hulp(klev)=hulp(klev)-flx_t(i) *max(0.0D+00, &
                    PELSURFACE(flx_states(i),1))/Depth(klev)/SEC_PER_DAY
             endif
          endif
        enddo
      ...
      if ( flx_option(nr).lt. 10 .or. flx_option(nr).ge.20) &
