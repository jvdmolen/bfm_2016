...
!BOC
   LEVEL1 fn
   iret = nf90_create(path=fn,cmode=nf90_clobber,ncid=ncid)
   call check_err(iret)
...
!
! !USES:
   use airsea,       only: tx,ty,I_0,heat,precip,evap,sst,sss,u10,v10
...
!  Time varying data : x,y,t
   ...
   iret = store_data(ncid,uwind_id,XYT_SHAPE,1,scalar=u10)
   iret = store_data(ncid,vwind_id,XYT_SHAPE,1,scalar=v10)
...
!  Time varying profile data : x,y,z,t
   ...
   iret = store_data(ncid,u_obs_id,XYZT_SHAPE,nlev,array=uprof(1:nlev))
   iret = store_data(ncid,v_id,XYZT_SHAPE,nlev,array=v(1:nlev))
   iret = store_data(ncid,v_obs_id,XYZT_SHAPE,nlev,array=vprof(1:nlev))
   iret = store_data(ncid,salt_id,XYZT_SHAPE,nlev,array=S(1:nlev))
   iret = store_data(ncid,salt_obs_id,XYZT_SHAPE,nlev,array=sprof(1:nlev))
   iret = store_data(ncid,temp_id,XYZT_SHAPE,nlev,array=T(1:nlev))
   iret = store_data(ncid,temp_obs_id,XYZT_SHAPE,nlev,array=tprof(1:nlev))
   ...
   do i=nlev-1,1,-1
      zz=zz+h(i+1)
      dz=real(0.5*(h(i)+h(i+1)))
      buoyp=real(eqstate1(sprof(i+1),tprof(i+1),zz/10.,gravity,rho_0))
      buoym=real(eqstate1(sprof(i  ),tprof(i  ),zz/10.,gravity,rho_0))
...
!  Time varying profile data : x,y,z1,t
   ...
   if (turb_method.ne.99) then
      ...
      iret = store_data(ncid,eps_obs_id,XYZT_SHAPE,nlev,array=epsprof(1:nlev))
   ...
   iret = store_data(ncid,o2_obs_id,XYZT_SHAPE,nlev,array=o2_prof(1:nlev))
...
! !INPUT PARAMETERS:
   ...
   integer, optional                   :: iarray(1:nlev)
   REALTYPE, optional                  :: scalar
   REALTYPE, optional                  :: array(1:nlev)
!
! !REVISION HISTORY:
!  Original author(s): Karsten Bolding & Hans Burchard
!
