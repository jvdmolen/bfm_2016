...
!  default: all is private.
   use bio_var
   use bfm_output,only:var_names,var_long,var_units, &
     stPelStateS,stPelStateE, stPelDiagS, &
     stPelDiagE, stPelFluxS, stPelFluxE, stBenStateS, stBenStateE, &
     stBenDiagS, stBenDiagE, stBenFluxS, stBenFluxE 
   private
...
!BOC
  ...
  91 close(unit)
     ivar=rriver+bdys(1)+bdys(2)+bdys(3)+bdys(4)+1
      ...
      return
  90 stop ' error: when  opening riverinfo.dat'

  92 stop ' error when reading bdyinfo.dat'
! 98 stop ' error when reading trace.inp'
! 99  FATAL 'I could not read trace.inp'
      stop 'int_bdy'
