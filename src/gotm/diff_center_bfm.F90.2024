...
!
! !INTERFACE:
   subroutine diff_center_bfm(N,dt,cnpar,posconc,h,Bcup,Bcdw, &
                          Yup,Ydw,nuY,Lsour,Qsour,Taur,Yobs,mode,Y)
!
! !DESCRIPTION:
...
! !USES:
   ...
   use mtridiagonal,only: au,bu,cu,du,tridiagonal
...
!  2: force mass-conservation 3: check deviation between input and output in Y
...
!
   if ( mode >= 2 ) then
       s=sum(h(1:N)*Y(1:N))
       if ( posconc.eq.1) s=s+(Yup+Ydw) * dt
   endif

!  set up matrix
   do i=2,N-1
      c     = 2.0D+00*dt*nuY(i)  /(h(i)+h(i+1))/h(i)
      a     = 2.0D+00*dt*nuY(i-1)/(h(i)+h(i-1))/h(i)
      l     =     dt*Lsour(i)
...
!   set up upper boundary condition
   select case(Bcup)
   case(Neumann)
      a     = 2.0D+00*dt*nuY(N-1)/(h(N)+h(N-1))/h(N)
      l     = dt*Lsour(N)
...
!   set up lower boundary condition
   select case(Bcdw)
   case(Neumann)
      c     = 2.0D+00*dt*nuY(1)/(h(1)+h(2))/h(1)
      l     = dt*Lsour(1)
...
!  if ( mode.eq.3) then
!    shelp=Y(1:N)
!    call findnan(shelp,N,i)
!    if (i > 0 ) STDERR 'diff_center before tridiagnal Nan'
!  endif
   call tridiagonal(N,1,N,Y)
!  if ( mode.eq.3) then
!    shelp=Y(1:N)
!    call findnan(shelp,N,i)
!    if (i > 0 ) STDERR 'diff_center after tridiagnal Nan'
!  endif

   if ( mode == 2 ) then
      s2=sum(h(1:N)*Y(1:N))+1.0D-80
      if ( mode .eq. 3) then
        s1=abs(s- s2)/(1.0D-80+s)/(dt/86400.0)
        if ( s1 > 1.0D-2 ) STDERR ' No balance in diff_center diff=',s1*100.0,'% /day'
      endif
      Y=Y*(s/s2)
    endif
   ...
   return
   end subroutine diff_center_bfm
!EOC
