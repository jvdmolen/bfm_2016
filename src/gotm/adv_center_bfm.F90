...
!
! !INTERFACE:
   subroutine adv_center_bfm(statenr,N,dt,h,ho,ww,Bcup,Bcdw,Yup,Ydw, &
                                                                method,mode,Y)
!
! !DESCRIPTION:
!
...
!  gradients, so-called slope limiters are applied for the factors $\Phi^+_{i}$
...
!
! !USES:
   use util,ONLY:UPSTREAM,P1,P2,SuperBee,MUSCL,P2_PDM,flux,value,oneSided,zeroDivergence
   use bio_var, ONLY:adv_courant,adv_number
   IMPLICIT NONE
!
! !INPUT PARAMETERS:
...
!  initialize interface fluxes with zero
   ...
   if ( mode.ge.2 ) s=sum(h(1:n)*Y(1:N))
...
!  compute maximum Courant number
   do k=1,N-1
      c=abs(ww(k))*dt/(0.5D+00*(h(k)+h(k+1)))
      if (c.gt.cmax) then
          cmax=c; i=k
      endif
   enddo
...
!#ifdef DEBUG
   if (it .gt. 1) then
      adv_courant=adv_courant + cmax
      adv_number=adv_number + it
   endif
!#endif
...
!           compute Courant number
            c=ww(k)/dble(it)*dt/(0.5D+00*(h(k)+h(k+1)))
...
!        compute the flux-factor phi
         x    =  one6th*(1.0D+00-2.0D+00*c)
         Phi  =  (0.5D+00+x)+(0.5D+00-x)*r
...
!        limit the flux according to different suggestions
         select case (method)
            ...
            case (P1)
              FATAL  &
                  "P1 advection method not yet implemented, choose other method"
              stop  "adv_center.F90"
            case ((P2),(P2_PDM))
               if (method.eq.P2) then
                 ...
                 limit=&
                   max(_ZERO_,min(Phi,2.0D+00/(1.0D+00-c),2.0D+00*r/(c+1.D-10)))
              end if
            case (Superbee)
               limit=max(_ZERO_, min(_ONE_, 2.0D+00*r), min(r,2.D+00*_ONE_) )
            case (MUSCL)
               limit=max(_ZERO_,min(2.D+00*_ONE_,2.0D+00*r,0.5D+00*(1.0D+00+r)))
            case default
               LEVEL3 method
...
!        compute the limited flux
         cu(k)=ww(k)*(Yc+0.5D+00*limit*(1.0D+00-c)*(Yd-Yc))
      end do
...
!     vertical flow velocity and for settling of suspended matter.
      ...
      if (mode.eq.0) then ! non-conservative
         do k=1,N
            Y(k)=Y(k)-1.0D+00/dble(it)*dt*((cu(k)-cu(k-1))/h(k) &
                                     -Y(k)*(ww(k)-ww(k-1))/h(k))
         enddo
      else                ! conservative
         do k=1,N
   ...
   if ( mode.ge.2  ) then
      s2=sum(h(1:n)*Y(1:n))
      if ( mode.eq.3) then
        s1=abs(s- s2)/s/(dt/864000.0)
        if (s1 > 1.0D-6 ) STDERR  'No balance in adv_center diff=',s1*100,'%'
      endif
!     Y(1:n)=Y(1:n)*(s/s2)
   endif
