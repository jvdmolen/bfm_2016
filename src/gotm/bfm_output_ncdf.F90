...
!
! !INTERFACE:
     integer function special_dims(mode,ncid,n,name,extname,units, &
...
!EOP
     ...
     integer function set_att_averaged(ncid,vars_id)
#ifdef NETCDF_FMT
       use netcdf
#endif
     IMPLICIT NONE
     ...
     REAL_4B                   :: vals(2)
       ...
       set_att_averaged=status
     return
     end function set_att_averaged
!-----------------------------------------------------------------------
!BOP
...
!            stPelStateS,stPelDiagS,stPelFluxS,stBenStateS,stBenDiagS, &
!            stBenFluxS,stPelStateE,stPelDiagE,stPelFluxE,stBenStateE, &
!            stBenDiagE,stBenFluxE, &
!            var_ids,var_names,var_long,var_units
     use output, only: out_fmt,ts
#ifdef INCLUDE_DIAGNOS_PRF
     use bio_var,only:diagb_prf,nprf   !,stPRFDiagS,stPRFDiagE
#endif
#ifdef NETCDF_FMT
     use netcdf
     use ncdfout, only: ncid
     use gotmout, only: lon_dim,lat_dim,z_dim,time_dim,dims_4
     use ncdfout, only: define_mode,new_nc_variable,set_attributes,store_data
#endif
use global_mem, ONLY:LOGUNIT  !JM
     IMPLICIT NONE
!
! !INPUT PARAMETERS:
!
...
!    integer,external          :: set_att_averaged
!EOP
!-----------------------------------------------------------------------
!BOC
       select case (out_fmt)
            ...
            if (bio_setup/=2) then
              ...
              do n=stPelStateS,stPelFluxE
                if ( var_ids(n) /= 0 )  then
                  ...
                  if (var_ave(n) )j= set_att_averaged(ncid,var_ids(n))
                end if
              end do
            end if
...
#ifdef INCLUDE_DIAGNOS_PRF
!           write(LOGUNIT,*) 'bio_save;stPRFDiagS,stPRFDiagE:',stPRFDiagS,stPRFDiagE
            do n=stPRFDiagS,stPRFDiagE
              j=0;if ( var_ids(n) /= 0 ) &
              j=special_dims(2,ncid,nprf,var_names(n),var_long(n), &
                       var_units(n),lon_dim,lat_dim,time_dim,var_ids(n))
!             write(LOGUNIT,*) 'special_dims j=',j,var_ids(n),var_names(n)
              if (var_ave(n) )j= set_att_averaged(ncid,var_ids(n))
            enddo
#endif
         ...
         do n=stPelStateS,stPelStateE
            if ( (var_ids(n) > 0) .and. (.not.var_ave(n) )) iret =  &
                 store_data(ncid,var_ids(n),XYZT_SHAPE,nlev,array=cc(1:nlev,n))
...
! stored as scalar: to be modified if benvar are arrays
          if (bio_setup>1) then
            ...
            do n=stBenStateS,stBenStateE
              i=i+1
              if ( (var_ids(n) > 0)  .and. (.not.var_ave(n))) iret = &
                store_data(ncid,var_ids(n),XYT_SHAPE,1,scalar=ccb(1,i))
            end do
            i=0
            ...
            do n=stPRFDiagS,stPRFDiagE
              ...
              if ( (var_ids(n) > 0).and. (.not.var_ave(n) ) ) &
               iret = store_data(ncid,var_ids(n),XYZT_SHAPE,nprf, &
                                        array=diagb_prf(1:nprf,i))
            end do
            do n=stPRFDiagS,stPRFDiagE
              i=i+1
              if ( (var_ids(n) > 0).and. var_ave(n) ) &
               iret = store_data(ncid,var_ids(n),XYZT_SHAPE,nprf, &
                                        array=ccb_ave_prf(1:nprf,i))
            end do
#endif
          end if
       end if !bio_model
