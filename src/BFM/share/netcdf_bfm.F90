...
!  Storing the time - both the coordinate and later a time string.
   ...
   do n=stPelFluxS,stPelFluxE
      i=i+1
      if ( var_ids(n) > 0 ) then
         call make_flux_output(1,i,NO_BOXES, Depth, c1dimz)
         iret = store_data(ncid_bfm,var_ids(n),OCET_SHAPE,NO_BOXES,garray=c1dimz)
      end if
   end do
   !---------------------------------------------
   ...
   if (bio_setup>1) then
      ...
      do n=stBenFluxS,stBenFluxE
         i=i+1
         if ( var_ids(n) > 0 ) then
            call make_flux_output(2,i,NO_BOXES_XY, Depth, c1dimz)
            iret = store_data(ncid_bfm,var_ids(n),BOTT_SHAPE,NO_BOXES_XY,garray=c1dimz)
         endif
      end do
      !---------------------------------------------
