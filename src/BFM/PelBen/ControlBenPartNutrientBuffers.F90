...
! !USES:
  ...
  use global_mem, ONLY:RLEN,ZERO,NZERO
  use mem,  ONLY: Q6n, D9m, D7m, Q6p, D8m, Q6s, D2STATE
  use mem, ONLY: ppQ6n, ppD9m, ppD7m, ppQ6p, ppD8m, ppQ6s, &
    PelBoxAbove, BoxNumber, BoxNumberXY,NO_BOXES_XY,  &
    jbotN4n, jbotN1p, jbotN5s, N4n_Ben, Depth_Ben, jbotR6n, &
    N1p_Ben, jbotR6p, N5s_Ben, jbotR6s, iiBen, flux
...
!
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ! In the module there is checked if one of the nutrients need control too 
  ! avoid too lowe values.if control_one==.TRUE. at least one of the 
  ! parameters has the value true.
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

  if (.NOT.p_control_one) return

  DO BoxNumberXY=1,NO_BOXES_XY
    BoxNumber=PelBoxAbove(BoxNumberXY)
    !----N:----------------------------------------------------------------
      if ( p_control_n) then
      rate = min( p_rn, p_s* N4n_Ben(BoxNumberXY)* Depth_Ben(BoxNumberXY))
      if ( -jbotR6n(BoxNumberXY)< p_rn .AND. &
                            Q6n(BoxNumberXY)/ rate< p_refill_time) then
        rate  =   max(  ZERO,  rate+ jbotR6n(BoxNumberXY))
        call flux(BoxNumberXY, iiBen, ppQ6n, ppQ6n, -(- rate) )
        call flux(BoxNumberXY, iiBen, ppD7m, ppD7m, ( ZERO- &
          D7m(BoxNumberXY))* rate/( NZERO+ Q6n(BoxNumberXY)) )
    ...
    if ( p_control_p) then
      ...
      if ( -jbotR6p(BoxNumberXY)< r .AND. &
                            Q6p(BoxNumberXY)/ rate< p_refill_time) then
        rate  =   max(  ZERO,  rate+ jbotR6p(BoxNumberXY))
        call flux(BoxNumberXY, iiBen, ppQ6p, ppQ6p, -(- rate) )
        call flux(BoxNumberXY, iiBen, ppD8m, ppD8m, ( ZERO- &
          D8m(BoxNumberXY))* rate/( NZERO+ Q6p(BoxNumberXY)) )
    ...
    if ( p_control_s) then
      ...
      if ( -jbotR6s(BoxNumberXY)< r .AND. &
                            Q6s(BoxNumberXY)/ rate< p_refill_time) then
        ...
        call flux(BoxNumberXY, iiBen, ppD9m, ppD9m, ( ZERO- &
          D9m(BoxNumberXY))* rate/( NZERO+ Q6s(BoxNumberXY)) )
