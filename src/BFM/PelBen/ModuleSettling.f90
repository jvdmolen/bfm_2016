...
!
  ...
  real(RLEN)  :: p_burvel_R6cnp=0.0  ! Burial Velocity (m/d)
  real(RLEN)  :: p_burvel_R6s=0.0  ! Burial Velocity (m/d) of skeletons of diatoms
  real(RLEN),dimension(iiPhytoPlankton)  :: p_burvel_PI  ! Burial Velocity (m/d)
  real(RLEN)  :: p_smRZ=0.0       ! FaecelPellet Degradation rate
  real(RLEN)  :: p_height=0.01    ! Height in which average concentrations is estimated for sedimentation
  real(RLEN)  :: p_xresusQ6=1.0    ! 
  real(RLEN)  :: p_xlimDm=1.0    ! 
  real(RLEN)  :: p_hDQ6m=30.  ! max value  D6n,D7m,D8m,D9m
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  namelist /Settling_parameters/ p_burvel_R6cnp,p_burvel_R6s, &
     p_burvel_PI,p_smRZ,p_height,p_xresusQ6,p_xlimDm,p_hDQ6m
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
