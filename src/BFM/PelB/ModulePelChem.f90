...
!
  ...
  real(RLEN)  :: p_sN4N3
  real(RLEN)  :: p_q10N4N3
  real(RLEN)  :: p_cyn=0.882 !p_cyn is lent from BenNbac mode: 
                       ! relation between C-Fixation and nitrifiaction
  real(RLEN)  :: p_q10R6N5
  real(RLEN)  :: p_sOS
  real(RLEN)  :: p_clO2o
  real(RLEN)  :: p_clN6r
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  namelist /PelChem_parameters/ p_sN4N3, p_q10N4N3, p_q10R6N5, p_sOS, p_clO2o, &
    p_clN6r, p_pN3O4, p_sR6N5,p_cyn
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
