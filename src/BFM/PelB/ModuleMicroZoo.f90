...
! !USES:
  ...
  use global_mem
  use mem,  ONLY: iiMicroZooPlankton, iiZ5, iiZ6,iiPhytoPlankton
...
!
  ...
  real(RLEN)  :: p_suBa(iiMicroZooPlankton)  ! /day   sw include B1 nitrifieres
  real(RLEN)  :: p_qnc(iiMicroZooPlankton)  ! Maximum quotum P
  real(RLEN)  :: p_qpc(iiMicroZooPlankton)  ! Maximum quotum N
  real(RLEN)  :: p_suPI(iiMicroZooPlankton,iiPhytoPlankton)  ! /day   #relative P1 uptake by zoo
  integer     :: p_type(iiPhytoPlankton)          ! group different type of food.
  real(RLEN)  :: p_suZI(iiMicroZooPlankton,iiMicroZooPlankton)  ! /day   #relative Z5 uptake by zoo0
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  namelist /MicroZoo_parameters/ p_q10, p_srs, p_sum, p_sdo, p_sd, p_pu_ra, &
    p_pu_ea, p_chro, p_chuc, p_minfood, p_suPI,p_type, p_suZI, p_suB1, p_suBa, &
    p_qpc, p_qnc
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

  !BEGIN compute
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ! if the values on 1 we have the "classical BFM model"
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   p_type=0
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
