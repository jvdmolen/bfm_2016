...
!
  ...
  integer     ::p_flux_at_deep_end  ! 0=no flux, 1= only fluxes_downwards (sink), 2,=full_flux

  integer     ::flag_ae_reset=1  !flag to reset alkalinity in oxic layer if pH is too high
  integer     ::flag_an_reset=1  !flag to reset alkalinity in anoic + denitrification  
                                  !layer if pH is too high
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  namelist /BenAlkalinity/ p_p,p_flux_at_deep_end,flag_ae_reset,flag_an_reset
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
