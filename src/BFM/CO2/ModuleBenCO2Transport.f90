...
!
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  !                 p_diff !if diff >0  moleculariCO2 diffusion is set and the 
                 ! standard temperature
                 ! if p_diff==0 calculation of diffusion  according 
                 ! Zeebe,RE(2011) is applied 
  real(RLEN)  :: p_p  ! adsorption distribution coefficeint
  integer     :: p_flux_at_deep_end=2 ! 0=no flux, 1= only fluxes_downwards 
                                       !(sink), 2,=full_flux (default)
              !to describe uptake of DIC by primary producers
              !if uptake/DIC mass devided by diffusion constant (m2/day) is larger then p_xcheck
              ! (the sqrt of this coefficient determines the gradient of the uptake fluxes)
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  namelist /BenCO2Transport_parameters/ p_p,p_flux_at_deep_end
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
