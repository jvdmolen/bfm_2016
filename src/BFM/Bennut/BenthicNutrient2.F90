...
! !USES:
  ...
  use mem,only: reBTo,rrBTo,Source_D2_vector,ppG2o,iiConsumption,iiProduction
!  
!
! !AUTHORS
...
!
  ...
  ! the nutrients
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  rrBTo=Source_D2_vector(ppG2o,iiConsumption)
  reBTo=Source_D2_vector(ppG2o,iiProduction)
