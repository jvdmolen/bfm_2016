...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      REAL(RLEN) FUNCTION CalculateSet(NUTR, mode,option,input,&
        ...
        if (NUTR /= nutr_seq) stop 'error: wrong use of CalculateSet'

        !calculate coefficients.......
        if (ns%nn == 0.or.nn_boundaries == 0) then
          write(0,'(''NUTR='',i4,'' ns%nn='',i4,'' equations='',i4)') &
                                      NUTR,ns%nn,nn_boundaries
          stop 'ns%nn=0 or nn_boundaries equation=0'
        elseif (ns%nn.ne.nn_boundaries) then
          write(0,'(''NUTR'',i4,'' termsn='',i4,'' equations='',i4)') &
                                              NUTR,ns%nn,nn_boundaries
        endif
        ...
        return
  100   call PrintSet( NUTR,"Singular matrix in ludcmp")
        call BFM_ERROR("CalculateSet","")
        CalculateSet=-1.0;
        return
      end
