...
SUBROUTINE PrintSet(NUTR,msg)
     ...
     write( LOGUNIT, &
    '(/,7X,''nr'',6X,''diffusion'',7X,''porosity'',5X,''adsorption   Depth'')')
     do i= 1,sets(NUTR)%equa
       write(LOGUNIT,'(''layer'',I6,1X,G16.6,1X,G13.6,1X,G13.6,1X,G13.6)') i, &
     ...
     do i= 1,sets(NUTR)%nn
       ...
       if ( l>=0)  then
         ...
         write(LOGUNIT,'(I2,4x,I2,3x, i2,G16.6,1X,2(G13.6,1X))') &
            i,k,l,sets(NUTR)%factor(i),  &
