...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      SUBROUTINE CompleteSet(NUTR,mode,option,input,xinput,value,mfac)
        USE global_mem, ONLY:RLEN,ALLOC,error_msg_prn,DONE,ZERO,LOGUNIT
        ...
        if (NUTR /= nutr_seq) stop 'error: wrong use of CompleteSet'

        select case (ns%status)
          case(DEFINE)
              nn_boundaries=0
              call DefineSet(NUTR,SET_BOUNDARY,0,0,ZERO,ZERO)
              C=ZERO
              Y2=ZERO
        ...
        select case (mode)
          ...
          case (SET_LAYER_INTEGRAL,SET_LAYER_INTEGRAL_UNTIL,& 
                  ADD+SET_LAYER_INTEGRAL,ADD+SET_LAYER_INTEGRAL_UNTIL )
            t=DONE
            s=multi
            if ( mode < ADD ) nn_boundaries=nn_boundaries+1
            do m=option,input
              if (ModeMass) t=ns%poro(m)*(ns%ads(m)+DONE)
              r=ns%b(m)
              u=ns%b(m+1)
!JM              if (mode == SET_LAYER_INTEGRAL_UNTIL.or. &
!JM                            ADD+SET_LAYER_INTEGRAL_UNTIL) then
              if (mode == SET_LAYER_INTEGRAL_UNTIL.or. &
                      mode ==  ADD+SET_LAYER_INTEGRAL_UNTIL) then
                if (m == input) u=xinput
              endif
              do k=m,m+1
                s=-s
        ...
        if (lowlevel_error>0) then
          write(LOGUNIT,*) 'lowlevel error in calls deon by CompleteSet' 
          write(LOGUNIT,*) 'NUTR,mode,option,input:',NUTR,mode,option,input
          write(LOGUNIT,*) 'xinput,value,mfac:',xinput,value,mfac
          call BFM_ERROR("CompleteSet","")
        endif
        return
      end
