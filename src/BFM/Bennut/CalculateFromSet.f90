...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      REAL(RLEN) FUNCTION CalculateFromSet(NUTR,mode,input,from,to)
        ...
        select case (mode)
          case (SHIFT)
            r = CalculateShift(NUTR,input,from,to);
            s=sign(DONE,to-from)
            CalculateFromSet=max(ZERO,r*s) *s
          case (INTEGRAL,EXPONENTIAL_INTEGRAL)
             ...
             call FindLayerNr(NUTR,0,x_from,j,xb)
          ...
          case (EQUATION,DERIVATIVE,SDERIVATIVE)
             !find layer/equation
             call FindLayerNr(NUTR,0,from,j,xb)
             CalculateFromSet=CalculateFromLayer(NUTR,j,mode,input,from,ZERO)
        end select
        return
      end
