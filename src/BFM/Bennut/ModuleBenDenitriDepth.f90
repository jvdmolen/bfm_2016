...
!BOC
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  real(RLEN)  :: p_pK3G4n  ! fraction of denitrifiation which take place below D2m
  real(RLEN)  :: p_clM3n  ! concentration where below no significint 
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  namelist /BenDenitriDepth_parameters/ p_pK3G4n,p_clM3n,p_mD2m, &
            p_xmulti,sw_method 
