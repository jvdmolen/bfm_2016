...
!
  ...
  write(LOGUNIT,*) "# Allocating State Variables and Rates array ..."
     ...
     D3STATE= ZERO
     allocate(D3SOURCE(1:NO_D3_BOX_STATES,1:NO_D3_BOX_STATES,1:NO_BOXESD3STATE),stat=status)
     if (status /= 0) call error_msg_prn(ALLOC,"AllocateMem", "D3SOURCE")
     D3SOURCE = ZERO
     allocate(D3SINK(1:NO_D3_BOX_STATES,1:NO_D3_BOX_STATES,1:NO_BOXESD3STATE)&
      & ,stat=status)
     ...
     D2STATE= ZERO
     allocate(D2SOURCE(1:NO_D2_BOX_STATES,1:NO_D2_BOX_STATES,1:NO_BOXESD2STATE),stat=status)
     if (status /= 0) call error_msg_prn(ALLOC,"AllocateMem", "D2SOURCE")
     D2SOURCE = ZERO
     allocate(D2SINK(1:NO_D2_BOX_STATES,1:NO_D2_BOX_STATES,1:NO_BOXESD2STATE)&
      & ,stat=status)
     ...
     allocate(D3DIAGNOS_PRF(1:NO_D3_BOX_DIAGNOSS_PRF,1:40),stat=status)
