...
!
  ...
  ! Global Constants
  !=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  real(RLEN),   parameter :: DBL_MAX=1.7e+308_RLEN
  real(RLEN),   parameter :: DBL_MIN=2.2250738585072014e-308_RLEN
  real(RLEN),   parameter :: MAX_VAL_EXPFUN=log(DBL_MAX*1.0D-10)
  real(RLEN),   parameter :: MIN_VAL_EXPFUN=log(DBL_MIN*1.0D+10)
  integer,      parameter :: ECOLOGY=1  ! Base temperature for Q10
  integer,      parameter :: TRANSPORT=2
  real(RLEN),   parameter :: MW_C=12.0107_RLEN  ! Molecular weight Carbon
  real(RLEN),   parameter :: MW_N=14.0067_RLEN  ! Molecular weight Nitrogen
  real(RLEN),   parameter :: MW_P=30.973_RLEN  ! Molecular weight Phosphorus
  real(RLEN),   parameter :: MW_SI=28.0885_RLEN  ! Molecular weight Silica
  real(RLEN),   parameter :: MW_H=1.00794_RLEN  ! Molecular weight Silica
  real(RLEN),   parameter :: MW_O=15.999_RLEN  ! Molecular weight Silica
  ...
  real(RLEN),   parameter :: p_qnUc=0.166666666667 ! quotum in urea between N ( mmol/mol ) and C (mgC/mol)
  ...
  integer,      parameter :: DIST_EXPONENTIAL_TERM=-3
  ...
  integer,      parameter :: NUMBER_OF_PROFILES=14
  integer,      parameter :: NCOEFF=22
  integer,      parameter :: ANY=1
  integer,      parameter :: NEGATIVE=2
  integer,      parameter :: POSITIVE=3
  !=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  end module
!BOP
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
