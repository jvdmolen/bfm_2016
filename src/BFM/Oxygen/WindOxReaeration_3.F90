...
! !USES:
  ...
  ! BoxNumberXY, Wind
  ! The following Pelagic 1-d global boxvars  are used: ETW, cmO2o, Depth
  ...
  use mem, ONLY: ppO2o, BoxNumberZ, BoxNumberY, NO_BOXES_Y, NO_BOXES_Z, &
    BoxNumberX, NO_BOXES_X, BoxNumber, BoxNumberXY, Wind, ETW, cmO2o, Depth, &
    jsurO2o, iiPel, flux
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  use global_interface,   ONLY: SurfaceO2Diffusion
!  
!
! !AUTHORS
...
!
  ...
  DO BoxNumberY=1,NO_BOXES_Y
    DO BoxNumberX=1,NO_BOXES_X
      ...
      call SurfaceO2Diffusion(1,ETW(BoxNumber),Wind,Depth(BoxNumber), &
                      cmO2o(BoxNumber)- O2o(BoxNumber),jsurO2o(BoxNumberXY))
