...
!
  ...
  real(RLEN)  :: p_k ! p_k = 0.31 cm/h  * 1/100 m *  24h = 0.074 m/d
                     ! p_k=  0.41 m/d  (Duenas etal.,1986 GeoChem. J. Vol 20,61-69)
  real(RLEN)  :: p_saf ! p_saf: sensitivity analysis factor,
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  namelist /WindOxReaeration_3_parameters/ p_k,p_saf
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
