...
! !USES:
  ...
  use global_mem
  use mem,  ONLY: iiBenOrganisms, iiSuspensionFeeders,iiBenBacteria,&
                  iiBenPhyto
...
!
  ...
  real(RLEN)  :: p_vum(iiBenOrganisms)  !
  ...
  real(RLEN)  :: p_sra(iiBenOrganisms)  !
  real(RLEN)  :: p_sd(iiBenOrganisms)  !
  real(RLEN)  :: p_sd2(iiBenOrganisms)  !
  real(RLEN)  :: p_clO2o(iiBenOrganisms)
  real(RLEN)  :: p_Yn(iiBenOrganisms,iiBenOrganisms)  ! Y1 > YI
  real(RLEN)  :: p_Sn(iiBenOrganisms,iiSuspensionFeeders)  ! Y1 > YI
  real(RLEN)  :: p_Hn(iiBenOrganisms,iiBenBacteria)  ! H1 > YI
  real(RLEN)  :: p_Pn(iiBenOrganisms,iiBenPhyto)  ! PB1 > YI
  ...
  integer     :: p_xdry(iiBenOrganisms) ! p_xdry :animal stops grazing in case of dry tidal flat.
  integer     :: p_xsenso(iiBenOrganisms) ! p_xsens=1: animal only present in D1m! 
  real(RLEN)  :: p_cluH
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  namelist /BenOrganism_parameters/ p_q10, p_vum,p_su, p_chu, p_clu, &
    p_pue, p_pur, p_sr, p_sra, p_puQ6, p_pueQ6, p_cm, p_clm, p_sd, p_sd2, &
    p_clO2o, p_qn, p_qp, p_Yn, p_Sn, p_Hn,p_Pn,p_xdry,p_xsenso,p_cluH
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
