...
!
  ...
  real(RLEN)  :: p_irrY3  ! -       # Multiplication factor to correct influence 
                          !           of a group of Y3 on bioirrigation
  real(RLEN)  :: p_irrY4  ! -       # Multiplication factor to correct influence 
                          !           of a group of Y4 on bioirrigation
  ...
  real(RLEN)  :: p_cmirr  ! -       # maximum enhancement of diffusion due to bioirrigation
  real(RLEN)  :: p_limDim !    # limit changes of D6m,D7m,D8,and D9m
  integer     ::sw_irr=1  !      sw_irr=1 irrigation caused by holes digged by macrobenthos
                          !      sw_irr=2 irrigation caused by uptake of oxygen from pelagic
                          !      sw_irr=3  a combination
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ! parameters which  control the oxygen uptake frok pelagic which induces
  ! the higher bioirrigation
  real(RLEN)   :: p_pu_xoxPelBen  ! max.proportion between oxygen uptake of pelagic and benthos
  real(RLEN)   :: p_xeff_oxuptake ! efficiency oxuptake by depostifeeders
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  namelist /Bioturbation_parameters/ p_q10, p_chtur, p_chirr, p_turY1, p_irrY3, p_irrY4, &
    p_Etur, p_cturm, p_cmtur, p_cmirr,p_limDim,sw_irr,p_pu_xoxPelBen,p_xeff_oxuptake
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
